...
3. **Строки аварий** (через tab):
   - **одиночная авария**: состояние | когда открыта | когда закрыта | текст | id
   - **в группе из нескольких аварий** (включая групповую): состояние | **объект** | когда открыта | когда закрыта | текст (без id).
