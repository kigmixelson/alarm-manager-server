...
Отдельный процесс опрашивает `POST /process` и печатает группы: название владельца аварии (при одном `parent_id` — с именем родителя в скобках, при нескольких — «влияет на N родительских объектов»), затем ссылки на аварии. Между группами — пустая строка.
